...
يجيب عن سؤالين منفصلين تماماً:
  ...
  (2) **هل تبديل الحساب يرسل إدخالات المالك باسم العامل؟** `flushQueue`
      ...
      وقت الإرسال، والصفوف لا تحمل هوية مُنشئها. لكن **النتيجة المقيسة
      (2026-09-05): لا يقع نَسْب خاطئ فعلياً** — الصف يحمل رمز CSRF لجلسة
      المالك، فيُرفض بـ400 تحت جلسة العامل ولا يُسجَّل أي بلاغ باسمه.
...
def main() -> int:
    with sync_playwright() as pw:
        ...
        # لا نَسْب خاطئ — رمز CSRF لجلسة المالك يُرفض تحت جلسة العامل
        check(not misattributed, "إدخال المالك لم يُنسَب للعامل (CSRF يرفض إعادة الإرسال)")
