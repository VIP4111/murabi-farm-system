...
**السؤال الذي يجيب عنه**: رفع `CACHE_NAME` لا يكفي وحده كدليل على
...
**السيناريو** (نفس الأصل، نفس سياق المتصفح — ترقية حقيقية لا محاكاة):
  ...
  4. نعيد `sw.js` للنسخة الجديدة (نفس ما يحدث لحظة النشر — يُقدَّم من القرص).
...
**النتيجة الموثَّقة (2026-09-05)**: نعم — بمجرد تفعيل النسخة الجديدة (بفضل
...
**لكن** ذلك يقع عند **التفعيل**، لا عند لحظة النشر — راجع الخلاصة المطبوعة.
...
def main() -> int:
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            print("\n2) نشر النسخة الجديدة (استبدال الملف على القرص)")
            SW_PATH.write_text(new_sw, encoding="utf-8")
            time.sleep(0.5)

            print("3) المتصفح يفحص التحديث ويفعّل النسخة الجديدة")
            ...
            check("murabi-offline-v7" not in after, "كاش v7 اختفى بعد تفعيل النسخة الجديدة",
                  str(list(after.keys())))
    ...
    print("  • لحظة النشر لا تكفي: الجهاز يظل على نسخته القديمة حتى يجلب /sw.js.")
    ...
    print("  • بفضل skipWaiting()+clients.claim() تتولّى الجديدة التبويبات فوراً،")
    print("    ومعالج activate يمسح كل كاش باسم مختلف — فيختفي v7 بكل محتواه.")
    print("  • النافذة المتبقّية: تنقّل واحد قد يُخدَم من القديمة قبل التفعيل، ولو كان")
    print("    الجهاز بلا اتصال تماماً فلن يُجلب /sw.js أصلاً وتبقى القديمة عاملة.")
    print("  ⇒ إجراء المستخدم بعد النشر على جهاز مشترك: افتح الموقع مرة وأنت")
    print("    متصل (تنقّل واحد يكفي لجلب /sw.js الجديد).")
    print("  ⇒ الخروج والدخول **وحدهما ليسا دليلاً** على الترقية: قد")
    print("    ينفّذهما الـSW القديم بلا أي ترقية. الدليل الوحيد المقبول هو تشغيل")
