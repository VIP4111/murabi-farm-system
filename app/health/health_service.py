...
# كتالوج التحصينات المرجعي (بند إضافي 290) — طلبك الصريح "جدول تحصين"
# بعد بحث خارجي فعلي (مصادر بيطرية + تقرير وزاري سعودي عن التحصينات
# الموسمية). عمداً بدون `default_dose_ml` ولا `withdrawal_days` مفترَضة
# — الجرعة وفترة السحب الفعليتين تعتمدان على المنتج التجاري المحدد
# اللي يشتريه الدكتور، ونظام هذا المشروع كله مبني على قاعدة "ما يخترع
# جرعة دواء" (بند 13 وتكرارها بكل مكان). `protection_days` هنا فقط
# مدة الحماية الموثّقة بالمصدر (تُستخدم لجدولة "الموعد القادم" تلقائياً
# بشاشة التحصين الجماعي، بند 61 — لا علاقة لها بحساب جرعة).
DEFAULT_VACCINE_CATALOG = [
    {
        "name": "لقاح CDT (تسمم دموي معوي + كزاز)",
        "protection_days": 365,
        "notes": (
            "اللقاح شبه المتفق عليه عالمياً لكل الأغنام/الماعز ضد Clostridium "
            "perfringens (النوعين C وD) والكزاز. الحملان: أول جرعة عمر 4-8 "
            "أسابيع + جرعة معززة بعد 3-4 أسابيع، ثم تحصين سنوي. النعاج الحوامل: "
            "قبل الولادة بـ4-6 أسابيع (تنتقل المناعة للمولود عبر اللبأ)."
        ),
    },
    {
        "name": "لقاح مشترك (طاعون المجترات الصغيرة PPR + جدري الأغنام)",
        "protection_days": 365,
        "notes": (
            "المرضان متوطّنان بالسعودية — دراسة مسحية سجّلت أعلى انتشار لـPPR "
            "بمنطقة الرياض (86%). اللقاح المشترك يعطي حماية موثّقة لمدة 12 شهراً "
            "من المرضين معاً. تُحصَّن القطعان عادة بفصل الربيع حسب توجيه الجهات "
            "الزراعية المحلية."
        ),
    },
    {
        "name": "لقاح الحمى القلاعية (FMD)",
        "protection_days": None,  # مدة الحماية تختلف حسب المنتج — راجع نشرته
        "notes": (
            "مرض متوطّن بالسعودية، شديد العدوى. **إلزامي لأي رأس مستورد/مشترى "
            "جديد أثناء فترة الحجر الصحي، قبل الاختلاط بالقطيع** — خصوصاً لو "
            "حالته التحصينية غير معروفة. مدة الحماية ومواعيد التكرار تختلف حسب "
            "المنتج التجاري المحدد — راجع نشرة المنتج مع دكتورك."
        ),
    },
]


def seed_default_vaccine_catalog() -> None:
    """يضيف أصناف الكتالوج أعلاه للصيدلية idempotent بالاسم (نفس نمط
    `Breed.seed_defaults`، بند 289) — رصيد صفر وسعر فاضي دائماً (ما
    نخترع سعر شراء)، والدكتور يعبّي الجرعة/السعر/فترة السحب الحقيقية
    لما يستقبل المنتج الفعلي أول مرة."""
    for entry in DEFAULT_VACCINE_CATALOG:
        if Pharmacy.query.filter_by(name=entry["name"]).first():
            continue
        db.session.add(Pharmacy(
            name=entry["name"], medicine_class="vaccine", available_qty=0,
            protection_days=entry["protection_days"], notes=entry["notes"],
        ))
    db.session.commit()
