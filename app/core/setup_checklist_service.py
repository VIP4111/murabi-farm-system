...
def get_setup_checklist_items() -> list[dict]:
    return [
        {"label": "أضف أول حظيرة", "done": Barn.query.count() > 0, "endpoint": "core.barns_new"},
        {"label": "أضف أول رأس", "done": Animal.query.count() > 0, "endpoint": "core.animals_new"},
        {"label": "أضف عضو فريق (عامل/دكتور)", "done": User.query.count() > 1, "endpoint": "team.members_new"},
        {"label": "أضف أول دواء بالصيدلية", "done": Pharmacy.query.count() > 0, "endpoint": "health.pharmacy_new"},
        {"label": "أضف أول صنف علف", "done": Feed.query.count() > 0, "endpoint": "feed.items_new"},
    ]
