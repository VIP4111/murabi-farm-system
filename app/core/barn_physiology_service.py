...
وحظيرته الحالية مو من النوع المناسب (`Barn.barn_type == "حمل متأخر"`
...
def generate_barn_move_tasks(*, now: datetime | None = None) -> list:
    """يولّد مهمة "🔀 انقل" لكل رأس وصل حالة (حمل متأخر/رضاعة) وحظيرته
    الحالية مو من النوع المطابق — بشرط وجود حظيرة فعلية بهذا النوع
    أصلاً (بدون كذا ما فيه وجهة ننقل لها). ترجع فقط المهام اللي
    أُنشئت الآن."""
    today = (now or datetime.now()).date()
    created = []

    targets = {
        "حمل متأخر": _late_pregnancy_animal_ids(today),
