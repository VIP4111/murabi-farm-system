...
class Pharmacy(db.Model):
    ...
    # توضيح مثل علبة كم ملي، عدد العلب" — عوض ما تكتب الكمية الإجمالية
    # مباشرة (رقم مجرَّد صعب التحقق منه بصرياً وقت الجرد)، تكتب عدد
    # العلب وكمية العلبة الواحدة، والنظام يحسب `available_qty` تلقائياً
    # (عدد العلب × كمية العلبة). **اختياري تماماً بالكامل** — لو فاضيين،
    # `available_qty` يبقى يُدخَل مباشرة يدوياً كالسابق (صفر كسر لأي
    # دواء مسجَّل مسبقاً بدون هذي التفاصيل). القيمتان تُخزَّنان فقط
    # لعرضهما وقت التعديل لاحقاً (نفس تفاصيل الجرد اللي كتبها المستخدم)
    # — `available_qty` يبقى دائماً مصدر الحقيقة الفعلي المستخدَم بكل
    # منطق الخصم/الحظر بالنظام، بدون أي تغيير.
    box_count = db.Column(db.Integer, nullable=True)
    box_quantity = db.Column(db.Float, nullable=True)
    ...
    def deduct_stock(self, qty: float) -> None:
        ...
        بند إصلاح (فحص "قسم الصحة/الأدوية") — `qty` نفسه ما كان يُتحقَّق
        إنه موجب: قيمة سالبة (كمية استخدام سالبة بخطأ إدخال أو طلب مُعدَّل
        يدوياً) كانت تمرّ من فحص `qty > available` بسهولة (سالب دايماً
        أصغر من أي رصيد موجب)، ثم `available_qty = available - qty`
        فعلياً **يزيد** المخزون بدل ما يخصمه — ثغرة حقيقية تسمح بتضخيم
        المخزون عبر تسجيل "استخدام" بكمية سالبة."""
        if qty <= 0:
            raise ValueError(_("الكمية المستخدمة لازم تكون رقماً موجباً أكبر من صفر."))
