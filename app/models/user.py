...
class User(UserMixin, db.Model):
    ...
    # تخصص/مسمى وظيفي إضافي (بند إضافي 267، طلبك الصريح: عندك عامل
    # مسؤول عن زراعة الأعلاف بالمزرعة، وبالتقارير كلهم يطلعون "عامل"
    # بس بدون تفريق). **مقصود منفصل تماماً عن الدور (Role)** — الدور
    # يبقى "عامل" لكل عمّالك زي ما هو (نفس الصلاحيات بالضبط، بدون أي
    # تغيير)، هذا الحقل وصف إضافي للعرض بالتقارير بس، يظهر جنب "عامل"
    # مو بدلها. اختياري، نص حر (زراعي، سائق، حارس...).
