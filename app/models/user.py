...
class User(UserMixin, db.Model):
    ...
    # بند إضافي (طلبك الصريح: "هل صاحب الحلال راح يكون مكفول جميع
    # العمال او في امكانيه تسجيل كل عامل بيناته لحالها") — قبل هذا
    # البند "صاحب العمل" بمسير الراتب كان يُطبع دائماً من بيانات صاحب
    # الحلال الوحيدة (`FarmSettings`) لكل عمال المزرعة بلا استثناء، حتى
    # لو عامل معيّن مكفول فعلياً على شخص/منشأة ثانية — يخلي المستند
    # مغلوطاً لو استُخدم رسمياً. هذي الحقول اختيارية بالكامل ولكل عامل
    # على حدة: فاضية (السلوك الافتراضي، ما تغيّر شي) = يستمر يُستخدم
    # صاحب الحلال تلقائياً كصاحب عمل (نفس القديم بالضبط)؛ معبّاة = تحل
    # محل بيانات صاحب الحلال بمسير راتب هذا العامل تحديداً بس.
    sponsor_name = db.Column(db.String(120), nullable=True)
    sponsor_national_id = db.Column(db.String(40), nullable=True)
    sponsor_phone = db.Column(db.String(32), nullable=True)
