...
class FarmSettings(db.Model):
    ...
    # رقم هوية صاحب العمل (بند إضافي 243) — لمسير راتب الشهر (نموذج
    # مشابه لمسيرات الرواتب الرسمية). تُدخَل يدوياً مرة وحدة وتُستخدم
    # تلقائياً بكل وصل راتب بعدها.
    ...
    # الحد الأقصى لعدد الأيام بين وزنتين متتاليتين لأي رأس نشط قبل ما
    # يُعتبر "وزن متأخر" ويولّد مهمة تلقائية (بند إضافي 149) — قيمة
    # اجتهادية مننا (30 يوم دورة شهرية معقولة)، عدّلها لو عندك دورة وزن
    # مختلفة.
    weight_check_interval_days = db.Column(db.Integer, default=30, nullable=False)
