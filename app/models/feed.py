...
class Feed(db.Model):
    ...
    # بند إصلاح (فحص عميق إضافي — طلبك: "ارجع افحص ولي ما طبقت عليها
    # طبقها") — `category` قائمة ثابتة فعلياً (خمس قيم، راجع القائمة
    # المنسدلة بـfeed/item_form.html)، مو نص حر كما بدا أول فحص. كانت
    # القائمة المنسدلة نفسها تستدعي `_(c)` بمتغيّر (مو نص ثابت)، وهذا
    # نمط ما تقدر أداة استخراج الترجمة (`pybabel extract`) تتبّعه
    # إطلاقاً — فالقيم الأربع غير "مركّز" ما كانت مسجَّلة بملفات
    # الترجمة أصلاً (خلل ترجمة كامن، منفصل عن خلل "لا تُعرض مترجَمة").
    # قاموس `_l()` ثابت (نفس نمط FEED_CLASS_LABELS_AR) يحل الاثنين معاً.
    FEED_ITEM_CATEGORY_LABELS_AR = {
        "مركّز": _l("مركّز"), "دريس": _l("دريس"), "تبن": _l("تبن"),
        "سيلاج": _l("سيلاج"), "إضافات": _l("إضافات"),
    }
