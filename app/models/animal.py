...
class Animal(db.Model):
    ...
    # السلالة (بند إضافي 51) — مصدرها الحقيقي الوحيد جدول `Breed`
    # (app/models/animal_options.py)، قابل للتوسعة من شاشة "+ حيوان
    # جديد" بزر "+" بدون أي تعديل كود. بند إضافي 291 — طلبك الصريح
    # "ابدأ" لما لقينا فجوة حقيقية: قائمة `Animal.BREEDS` الثابتة هذي
    # كانت لسا مستخدَمة بشاشة "استقبال دفعة" لحالها (مصدر ثانٍ منفصل
    # لنفس المفهوم — أي سلالة تُضاف بزر "+" ما كانت تظهر هناك إطلاقاً)،
    # فحُذفت نهائياً؛ الشاشتان الآن تقرآن من جدول `Breed` نفسه فقط.
    # بند إضافي 289 — "ماعز" وصف/تصنيف بس، ما تأثر على محرك دورة
