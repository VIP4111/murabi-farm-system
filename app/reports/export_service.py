...
def build_employee_of_month_receipt_pdf(record, farm_settings) -> io.BytesIO:
    """وصل استلام مكافأة/راتب موظف الشهر (بند إضافي 240) — مستند
    مفرد لشهر واحد بس (المكافأة اللي تأكَّدت فعلاً وتحوّلت)، بطلبك
    الصريح بعد ما شاركت مثال "مسير راتب" رسمي — نفس الفكرة العامة
    (بيانات صاحب العمل، بيانات المستلم، صف الشهر المستحق)، مبني من
    بيانات النظام الفعلية بس (بدون أي بيانات هوية/جواز غير متوفرة
    أصلاً بالنظام)."""
    _ensure_font()
    buf = io.BytesIO()
    c = canvas.Canvas(buf, pagesize=A4)
    ...
    c.setFont("Arabic", 18)
    c.drawRightString(right_margin, y, ar("وصل مكافأة موظف الشهر"))
    y -= 10 * mm
    c.setFont("Arabic", 10)
    c.drawRightString(right_margin, y, ar(f"التاريخ: {record.confirmed_at.date() if record.confirmed_at else ''}"))
    y -= 12 * mm
    ...
    y -= 6 * mm
    c.setFont("Arabic", 10)
    c.drawRightString(right_margin, y, ar(record.user.name))
    y -= 5.5 * mm
    if record.recipient_name:
        c.drawRightString(right_margin, y, ar(f"اسم المستلم (حوالة): {record.recipient_name}"))
        y -= 5.5 * mm

    y -= 12 * mm
    col_period = right_margin
    col_amount = left_margin + 30 * mm
    c.setFont("Arabic", 10)
    c.drawRightString(col_period, y, ar("الفترة"))
    c.drawRightString(col_amount, y, ar("المبلغ المستحق"))
    y -= 3 * mm
    c.line(left_margin, y, right_margin, y)
    y -= 8 * mm

    period_label = f"{_ARABIC_MONTHS.get(record.month, record.month)} {record.year}"
    c.setFont("Arabic", 10)
    c.drawRightString(col_period, y, ar(period_label))
    c.drawRightString(col_amount, y, ar(f"{record.bonus_amount:,.2f}"))
    y -= 6 * mm
    c.line(left_margin, y, right_margin, y)
    y -= 10 * mm

    c.setFont("Arabic", 13)
    c.drawRightString(right_margin, y, ar(f"الإجمالي المستحق: {record.bonus_amount:,.2f}"))
    y -= 10 * mm
    c.setFont("Arabic", 9)
    c.drawRightString(right_margin, y, ar(f"معتمَد من: {record.confirmed_by.name if record.confirmed_by else '-'}"))
