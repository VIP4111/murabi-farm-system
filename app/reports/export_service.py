...
# نص إقرار المخالصة الافتراضي (بند إصلاح — طلبك: "النصوص القانونية
# مسودة عامة. هل هاذي قابله لتعديل عن طريق البرنامج ولا ثابته" ثم
# "نعم خليه قابل لتعديل") — يُستخدم بس لو `FarmSettings.
# settlement_pledge_text` فاضي (سلوك افتراضي، صفر كسر لأي مزرعة ما
# عدّلت النص بعد). `{اسم_العامل}`/`{اسم_الكفيل}` يُستبدَلان تلقائياً
# وقت الطباعة بقيمهما الفعلية — سطر فاضي بين الفقرتين يفصلهما لسطرين
# منفصلين بالمستند المطبوع (نفس تقسيم بنود العقد: سطر لكل فقرة).
DEFAULT_SETTLEMENT_PLEDGE_TEXT = (
    "أقرّ أنا الموقّع أدناه ({اسم_العامل}) بأنني استلمت من الكفيل/صاحب العمل ({اسم_الكفيل}) "
    "كامل مستحقاتي المالية المذكورة أعلاه (الراتب ومكافأة نهاية الخدمة وأي بدلات مستحقة) عن كامل "
    "مدة عملي، وأنه لا يوجد لي أي مطالبة مالية أو حقوقية أخرى تجاه صاحب العمل بأي صفة كانت، سواء "
    "بشكل حالي أو مستقبلي، وذلك اعتباراً من تاريخ توقيعي على هذا الإقرار.\n"
...
def build_settlement_pdf(user, context, farm_settings) -> io.BytesIO:
    ...
    نص الإقرار صار قابلاً للتعديل الكامل من الإعدادات
    (`FarmSettings.settlement_pledge_text`، فاضي = النص الافتراضي
    `DEFAULT_SETTLEMENT_PLEDGE_TEXT`)، والمبالغ تُدخَل يدوياً وقت
    الطباعة (`context`) بدل حساب تلقائي لمكافأة نهاية الخدمة — بطلبك
    الصريح."""
    _ensure_font()
    buf = io.BytesIO()
    c = canvas.Canvas(buf, pagesize=A4)
    ...
    pledge_text = (farm_settings.settlement_pledge_text or DEFAULT_SETTLEMENT_PLEDGE_TEXT)
    pledge_text = pledge_text.replace("{اسم_العامل}", user.name).replace("{اسم_الكفيل}", employer_name)
    paragraphs = [p.strip() for p in pledge_text.split("\n") if p.strip()]
