...
def register_cli(app):
    ...
    def simulate_farm_month(days):
        ...
        from app.core import email_service, telegram_service
        ...
        if not owner:
            click.echo("ما فيه حساب مالك — شغّل `flask seed` أولاً.")
            return
        ...
        if not species or not breed or not color:
            click.echo("تعذّر تجهيز القوائم المرجعية (فصيلة/سلالة/لون).")
            return
        ...
        for offset in range(days):
            ...
            # شراء عشوائي (~20% من الأيام رأس أو رأسين)
            if random.random() < 0.2:
                for _ in range(random.randint(1, 2)):
                    _new_animal(day, random.choice(["ذكر", "أنثى"]))

            # مهمة يومية بسيطة، تُنجَز بمعدل 75% (يحاكي واقع فعلي — مو
            # 100% كامل دائماً، عشان بند "متكرر التجاهل" يعني شي فعلي)
            ...
            # تقريع (~10% من الأيام لو فيه ذكور وإناث كافية)
            ...
            # حالة مرضية (~8% من الأيام)، تُغلَق بعد فترة عشوائية (~60%
            # منها) لمحاكاة تعافي فعلي
            if random.random() < 0.2:
        ...
        # التحليل المالي ونقطة التعادل (بند 176) — على نفس البيانات
        # اللي تولّدت بالمحاكاة، يثبت الربط الفعلي بين الأحداث والتقرير.
        ...
        body_lines = [
            ...
            f"- بريد الإشعارات (Resend): {'مُفعَّل — هذا التقرير نفسه دليل الإرسال' if email_service._config() else 'غير مُفعَّل (RESEND_API_KEY/EMAIL_FROM_ADDRESS فاضيين) — هذا التقرير ما وصل فعلياً، طبعناه هنا بس'}",
            f"- تيليجرام: {'مضبوط' if telegram_service._bot_token() else 'غير مضبوط (TELEGRAM_BOT_TOKEN فاضي)'} — هذي المحاكاة ما تستدعي مسارات الإشعار الفوري لكل حدث (تقريع/مرض) لأنها تكتب البيانات مباشرة بدل تشغيل شاشات الويب كاملة؛ مسارات البريد/تيليجرام الفعلية مُتحقَّق منها ببنود سابقة (165-179) عبر مسار الطلب الحقيقي، مو هذا السكربت.",
            "",
        ...
        if sent:
            click.echo(f"✅ تم إرسال التقرير فعلياً لبريد {owner.email}.")
        else:
            click.echo(
