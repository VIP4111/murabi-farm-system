...
def create_app(config_class=Config):
    ...
    # فحص تشغيلي (بحث "مشاكل تشغيلية"): ما فيه أي errorhandler(500) — أي
    # استثناء غير متوقع (خطأ برمجي، انقطاع اتصال قاعدة البيانات...) كان
    # يطلع للمستخدم صفحة Flask بيضاء خام بالإنجليزي ("Internal Server
    # Error") بدل هوية النظام، بنفس مشكلة 403 القديمة بالضبط. أخطر من
    # هيك: لو سبب الخطأ استثناء بمنتصف عملية قاعدة بيانات (مثلاً commit
    # فشل)، جلسة db.session تبقى "متسخة" (transaction معلّقة) — أي طلب
    # ثاني بنفس السياق يفشل بخطأ غامض ثاني فوق الأول. rollback() هنا
    # ينظّف الجلسة قبل ما نعرض صفحة الخطأ (اللي بنفسها تستخدم قاعدة
    # البيانات عبر base.html للقوائم/العدادات).
    @app.errorhandler(500)
    def internal_server_error(e):
        db.session.rollback()
        return render_template(
            "error_page.html", icon="⚠️",
            title=_l("صار خطأ تقني غير متوقع"),
            message=_l("حاول مرة ثانية، ولو تكرر الخطأ أبلغ صاحب الحلال بوقت حصوله وشو كنت تسوي بالضبط عشان نصلحه."),
        ), 500

    # نفس فحص "المشاكل التشغيلية" لقى ثغرة ثانية بنفس النمط: MAX_CONTENT_LENGTH
    # (10MB) مضبوط لحماية السيرفر من رفع ملفات ضخمة، لكن ما فيه
    # errorhandler(413) — فلو مستخدم رفع صورة بلاغ أو ملاحظة صوتية أكبر
    # من الحد (احتمال وارد جداً بصور جوال حديث)، يطلع صفحة Flask بيضاء
    # خام "413 Request Entity Too Large" بدل رسالة عربية مفهومة توضّح
    # السبب والحل.
    @app.errorhandler(413)
    def request_entity_too_large_error(e):
        return render_template(
            "error_page.html", icon="📦",
            title=_l("الملف كبير جداً"),
            message=_l("حجم الملف اللي رفعته أكبر من المسموح — جرّب صورة أو تسجيل صوتي بحجم أصغر."),
        ), 413
