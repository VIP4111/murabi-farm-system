...
ENTRIES: list[KBEntry] = [
    ...
    KBEntry(
        ...
        body=(
            "'الحظائر' ← 'إضافة حظيرة': رقم الحظيرة (لازم يكون فريد)، الاسم، النوع، السعة، والعامل المسؤول عنها. من نفس الشاشة تقدر تفتح 'تعديل' لأي حظيرة موجودة — أهم استخدام عملي لها هو تغيير العامل المسؤول بعدين، لأنه أساس توجيه المهام التلقائي والتنبيهات لذاك العامل."
        ),
    ),
    KBEntry(
        ...
        body=(
            "من صفحة تعديل الدواء بالصيدلية ← 'تسجيل شراء': أدخل الكمية، تاريخ الشراء، تاريخ انتهاء هذي الدفعة تحديداً (ممكن يختلف عن دفعات سابقة)، والسعر لو حبيت. الرصيد الإجمالي (`available_qty`) يزيد تلقائياً بنفس الكمية، وتقدر تراجع كل دفعات هذا الدواء وتواريخها من نفس الصفحة. فورم 'تعديل' المباشر يبقى موجود لتصحيحات الرصيد العامة بس، مو بديل عن تسجيل الشراء."
        ),
    ),
    KBEntry(
