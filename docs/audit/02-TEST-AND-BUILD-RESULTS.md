...
| JS tests (node --test) | **Pass** | **22 passed، 0 failed** (`tests/js/sw.test.js`, `offline_sync.test.js`) — هذا رقم لحظة التدقيق؛ صار **56 passed** بعد دفعة `SEC-01` | `node --test tests/js/*.test.js` → `# pass 22 / # fail 0` (وبعد الدفعة: `# pass 56`) |
...
| البند | السبب |
|---|---|
| ~~السلوك الفعلي للـService Worker والكاش بين مستخدمين~~ | **أُغلقت 2026-09-05 (دفعة SEC-01)**: نُفِّذ العرض الحي بـChromium على سياق واحد (جهاز مشترك). على `main` @ 5de1eeb ظهرت صفحة مالية المالك للعامل بلا شبكة وفيها «صافي الربح»؛ وعلى فرع الإصلاح: صفر تسريب. صار اختباراً آلياً قابلاً للتكرار — `bash tests/e2e/run_sw_e2e.sh`. **وأُضيف 2026-09-05 (نفس الدفعة)**: قياس ردّ المالك مقابل العامل للمسارات الخمسة المستثناة (`--diff`) الذي أسقط الاستثناء كله، وإثبات ترتيب تبديل الهوية باحتجاز ردّ `/logout` نفسه (حجب فوري بلا إعادة قراءة، ثم إعادة تحميل بعد اكتمال الخروج) وحارس الحقبة برد محتجَز على مسار ضمن قائمة السماح — كلاهما مُثبت أنه **يسقط** على السلوك الخاطئ، وأداة فحص ما بعد النشر مُختبَرة بالاتجاهين (FAIL على نسخة قديمة، PASS بعد الترقية). |
